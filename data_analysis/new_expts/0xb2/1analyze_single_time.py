...
filename = ["partial_prog_test_6c.txt"]

no_of_pages_in_a_file = 512
no_of_bytes_in_a_page = 18592
no_of_time_instances = 25
...
for this_file_name in filename:
	...
	for each_time in range(no_of_time_instances):
		...
		f_out = open(os.path.join(output_dir,"slc_partialprogram_data_"+str(each_time*25)+"us.csv"),"w")
	...
	for each_page in range(no_of_pages_in_a_file):
		...
		for each_time in range(1,no_of_time_instances):
			with open(os.path.join(output_dir,"slc_partialprogram_data_"+str(each_time*25)+"us.csv"),"r") as f_in:
				data_for_this_page_time = f_in.read().split("\n")[each_page+1].split(",")
				f_out.write("\n"+str(each_time*25)+","+data_for_this_page_time[1]+","+data_for_this_page_time[2])
		f_out.close()