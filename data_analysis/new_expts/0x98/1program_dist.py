...
zero_ones = np.zeros((no_of_pages_in_a_file,no_of_reads,2))
...
for this_file_name in filename:
	...
	for each_read in range(no_of_reads):
		...
		for each_page in range(no_of_pages_in_a_file):
			...
			zero_position = page_data[each_page,each_read]==0
			zero_ones[each_page,each_read,0] = len(page_data[each_page,each_read][zero_position])
			one_position = page_data[each_page,each_read]==1
			zero_ones[each_page,each_read,1] = len(page_data[each_page,each_read][one_position])
	...
	for each_page in range(no_of_pages_in_a_file):
		...
		# for plotting the data characterized for each page at different stress levels
		out_01_data = os.path.join(folder1_name,"zero_ones_"+str(each_page)+".csv")
		np.savetxt(out_01_data,zero_ones[each_page],fmt='%1d',delimiter=",")
